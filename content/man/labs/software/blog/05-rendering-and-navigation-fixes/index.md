---
title: "05 — Fixing favicons, mathematics, links, and navigation"
description: "Root-cause analysis for four visible rendering and publication problems"
date: 2026-08-29T23:25:00+02:00
show_in_posts: true
weight: 60
---
...
## Posts: importing content did not imply publication

The timeline intentionally includes only:

- regular pages stored under `posts`;
- regular pages with `show_in_posts: true`.

The imported network articles had neither a timeline date nor the opt-in flag.
They were valid manual pages, so Hugo correctly rendered them under `man`, but
the shared collection correctly excluded them.

Each article received an explicit local timestamp and:

```yaml
show_in_posts: true
```

After that change, the same page objects appeared in the posts page, home page,
and RSS feed. No template exception for the network section was needed.

## Empty table-of-contents entry

The table of contents reflects the Markdown heading tree. Inconsistent heading
levels inherited from the export caused an item with no useful visible label.
Normalizing the structure to `##` sections and `###` subsections produced named
entries for:
...
The correction belongs in Markdown, not CSS. Hiding an empty list item would
mask malformed document structure and leave assistive technologies with the
same problem.

## Verification

The production output was checked for behavior, not just source presence:

- the home `<head>` contains PNG, manifest, SVG, and fingerprinted SVG favicon
  links;
- the hashed SVG exists in `public/icons/`;
- a math page contains KaTeX CSS, the core script, auto-render, and delimiter
  configuration;
- a non-math page contains no KaTeX assets;
- all network titles appear in `public/posts/index.html`;
- all canonical network URLs appear in `public/posts/index.xml`;
- the rendered table of contents contains no empty anchor;
- the final Hugo build succeeds without path warnings.

The general lesson is that visible rendering bugs should be traced through the
whole pipeline: source content, front matter, template condition, generated
HTML, resource existence, and browser caching.
